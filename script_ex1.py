...
for i in range(len(s)):
    if re.match(k, s[i:i+lk]):
        result.append((i,i+lk-1))
...
for _ in range(n):
    UID = input()
    if len(UID) == 10 and re.match(r'(.*[A-Z]){2,}', UID) and re.match(r'(?!.*(.).*\1)', UID) and re.match(r'(.*[0-9]){3,}', UID):
        print('Valid')
...
for _ in range(n):
    ...
    add.append(numpy.array(A+B,int))
    sub.append(numpy.array(A-B,int))
    mul.append(numpy.array(A*B,int))
    div.append(numpy.array(A//B,int))
    mod.append(numpy.array(A%B,int))
    power.append(numpy.array(A**B,int))

print(numpy.array(add))
print(numpy.array(sub))
print(numpy.array(mul))
print(numpy.array(div))
print(numpy.array(mod))
print(numpy.array(power))
